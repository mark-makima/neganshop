...
class TelegramAuth:
    ...
    async def _save_session_and_cleanup(self, client, phone, chat_id):
        ...
        try:
            ...
            clean_phone = re.sub(r'[^0-9]', '', phone)  # Удаляем все нецифровые символы
            session_dir = "/data/sessions"
            os.makedirs(session_dir, exist_ok=True)  # Создаем директорию, если ее нет
            session_file = f"{session_dir}/{clean_phone}.session"
            ...
            # Проверяем корректность сохраненной сессии
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if code.isdigit() and len(code) == 5:
                ...
                os.remove(f'/data/sessions/{phone_number}.session')
